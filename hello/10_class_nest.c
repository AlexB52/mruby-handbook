...
void define_foo_buz(mrb_state *mrb) {
  struct RClass *foo_class =
      mrb_define_class(mrb, "Greeting", mrb->object_class);
...
int main() {
  ...
  struct RClass *hello_module =
      mrb_module_get_under(mrb, parent_class, "Hello");

  struct RClass *world_class =
      mrb_define_class_under(mrb, hello_module, "World", mrb->object_class);
